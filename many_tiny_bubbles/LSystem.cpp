...
void LSystem::setRandAngleRange(float rand)
{
    randAngleRange = rand;
}
    ...
    std::vector<Geometry>& models)
{
    ...
    for (unsigned int i = 0; i < insn.size(); i++)
    {
		...
		srand(seed);
		if((int)randAngleRange <= 0)
			randNum = 0;
		else
			randNum = rand() % (int)randAngleRange;
		Sleep(1);
        ...
        {
            turtle.applyUpRot(mDfltAngle + randNum);
        }
        else if (sym == "-")
        {
            turtle.applyUpRot(-mDfltAngle + randNum);
        ...
        {
            turtle.applyLeftRot(mDfltAngle + randNum);
        }
        else if (sym == "^")
        {
            turtle.applyLeftRot(-mDfltAngle + randNum);
        ...
        {
            turtle.applyForwardRot(mDfltAngle + randNum);
        }
        else if (sym == "/")
        {
            turtle.applyForwardRot(-mDfltAngle + randNum);
